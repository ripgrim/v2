...
export const Route = createFileRoute("/$org/$repo/pulls/")({
	ssr: false,
...
function PullsListPage() {
	...
	const pulls = content.data?.pulls ?? [];
	const open = pulls.filter((p) => p.status === "open");
	const closed = pulls.filter((p) => p.status !== "open");
				...
				<div className="mx-auto flex w-full max-w-4xl flex-col gap-5">
					<div className="flex h-8 items-center">
						<RepoCrumbs org={org} repo={repo}>
							<CrumbSep />
							<CrumbText>Pulls</CrumbText>
						</RepoCrumbs>
					</div>

					<RepoTabs org={org} repo={repo} active="pulls" />
