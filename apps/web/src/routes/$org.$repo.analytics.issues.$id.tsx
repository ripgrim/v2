...
import { ArrowUpRight, CircleDot, Shield, Trash2 } from "lucide-react";
...
import { THREAD_STATUS } from "#/lib/repo-analytics.config";
import { repoAnalyticsQueryOptions } from "#/lib/repo-analytics.query";
import { MODERATOR } from "#/lib/site-config";
import { cn } from "#/lib/utils";

export const Route = createFileRoute("/$org/$repo/analytics/issues/$id")({
	ssr: false,
	...
	component: IssueAnalyticsPage,
});

function IssueAnalyticsPage() {
	...
	const thread = analytics.data?.threads[`issues/${id}`];
							...
							<div className="flex flex-col gap-6 sm:flex-row sm:items-start">
								...
								<section className="flex flex-1 flex-col gap-2.5">
									...
									<div className="flex flex-col">
										{thread.flagged?.map((f) => (
											<div
												key={`${f.login}-${f.reason}`}
												className="flex items-center gap-2.5 rounded-lg px-2.5 py-2 transition-colors hover:bg-surface-1"
											>
												{f.status === "Removed" ? (
													<Trash2
														size={15}
