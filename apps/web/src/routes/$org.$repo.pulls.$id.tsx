...
export const Route = createFileRoute("/$org/$repo/pulls/$id")({
	ssr: false,
...
function PullDetailPage() {
	...
	const detail = content.data?.pullDetails[id];
	const threadAnalytics = analytics.data?.threads[`pulls/${id}`];
				...
				<div className="mx-auto flex w-full max-w-3xl flex-col gap-6">
					...
					{detail ? (
						...
						<p className="py-16 text-center text-muted-foreground text-sm">
							{content.isLoading ? "Loading…" : "Pull request not found."}
						</p>
					)}
				</div>
			</ThreadDetailShell>
